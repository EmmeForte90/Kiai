...
public class nemico_bow : MonoBehaviour
{
  private GameObject toy; // Variabile per il player
    ...
    [SerializeField] Transform ShootP;
    ...
    [SpineAnimation][SerializeField] string aim;
    [SpineAnimation][SerializeField] string die;
    [SpineAnimation][SerializeField] string Shoot;
    [SpineAnimation][SerializeField] string rech;
...
public void AimANM()
{
    if (currentAnimationName != aim)
                {
                    spineAnimationState.SetAnimation(2, aim, false);
                    currentAnimationName = aim;
                }
...
{
    if (currentAnimationName != rech)
                {
                    spineAnimationState.SetAnimation(2, rech, false);
                    currentAnimationName = rech;
                }
                // Add event listener for when the animation completes
                spineAnimationState.GetCurrent(2).Complete += OnAttackAnimationComplete;
}
...
public void AttackAnm()
{
    if (currentAnimationName != Shoot)
                {
                    spineAnimationState.SetAnimation(2, Shoot, false);
...
void HandleEvent (TrackEntry trackEntry, Spine.Event e) 
{
    ...
    {     
     ...
     if(!vfx)
        {
        vfxTimer = 0.5f;
        Instantiate(Arrow, ShootP.transform.position, transform.rotation);
        vfx = true;
        }
    }
